...
cur = conn.cursor()
cur.execute("SELECT name, sql FROM sqlite_master WHERE type='table' AND name LIKE 'graph_%' OR name='previous_versions'")
...
for n in list(g_restart.nodes.values())[:5]:
    ...
    ident = getattr(e, 'ip', '') or getattr(e, 'domain_name', '') or getattr(e, 'cve_id', '') or n.id
...
t0 = time.perf_counter()
for i in range(500):
    g3.add_entity(IPAddress(id=f'ip_{i}', ip=f'10.{i//65536}.{(i//256)%256}.{i%256}', confidence_score=80))
t1 = time.perf_counter()
...
report = {
    ...
    "schema": {
        ...
        "graph_nodes_cols": ["node_id (PK)", "entity_type", "entity_data (JSON)", "confidence_score", "created_at", "updated_at"],
        "graph_edges_cols": ["edge_id (PK)", "source_node_id (FK)", "target_node_id (FK)", "relationship_type", "confidence", "edge_data (JSON)", "created_at"],
        "previous_versions_cols": ["version_id (PK AUTOINCREMENT)", "node_id", "version_data (JSON)", "entity_type", "saved_at"],
    },
    "persistence_test": {
...
print(f"    Persistence: nodes={nodes_match}, edges={edges_match}, types={types_match}, conf={confs_match}")
print(f"    Regression: in-memory OK ({len(g_mem.nodes)} nodes)")
print(f"    Performance: 100 nodes in-memory={mem_time:.3f}s vs storage={storage_time:.3f}s ({((storage_time-mem_time)/mem_time*100):.1f}% overhead)")
...
print(f"  Schema: graph_nodes + graph_edges + previous_versions (3 tablo)")
print(f"  Write-through: add_entity, add_relationship, remove_node, remove_edge, resolve_evidence_contradictions")
print(f"  Restart persistence: {restart_node_count}/{orig_node_count} nodes, {restart_edge_count}/{orig_edge_count} edges")
