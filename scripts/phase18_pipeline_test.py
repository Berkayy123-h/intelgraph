...
OVERLAP_CVES = {
    "cve-2015-2051", "cve-2016-4437", "cve-2017-11882", "cve-2017-9841",
    "cve-2018-0802", "cve-2018-15982", "cve-2018-20250", "cve-2021-26855",
...
matching_cves = [v for v in kev_data["vulnerabilities"] if v["cveID"].strip().lower() in OVERLAP_CVES]
kev_text = "\n".join(
...
# ── 4. Build sources ──
sources = [
    {"id": "urlhaus_overlap", "name": f"URLhaus ({len(matching_urls)} satir)", "text": urlhaus_text, "value": 60},
    {"id": "kev_overlap", "name": f"CISA KEV ({len(matching_cves)} CVE)", "text": kev_text, "value": 90},
]
for p in pulses:
...
for p in pulses:
    for ind in p.indicators:
        itype = ind.get("type", "")
        if "SHA256" in itype:  hash_counts["SHA256"] += 1
        if "MD5" in itype:     hash_counts["MD5"]    += 1
        if "SHA1" in itype:    hash_counts["SHA1"]   += 1
...
for nid, node in result.graph.nodes.items():
    if isinstance(node.entity, CveEntity):
        if node.entity.cve_id.lower() in OVERLAP_CVES:
            print(f"    {node.entity.cve_id}: conf={node.entity.confidence_score} ev={len(node.entity.evidence)} node_id={nid}")

# Show merge audit details for overlapping entities
print("\n  --- Merge audit (all) ---")
for ma in result_dict.get("merge_audit", []):
    print(f"    {ma.get('source_entity_id','')} -> {ma.get('target_entity_id','')} ({ma.get('merge_strategy','?')})")
...
if result.graph:
    for node in result.graph.nodes.values():
        if isinstance(node.entity, IPAddress):
            ip = node.entity.ip
            if re.match(r'^[a-fA-F0-9]{32}$', ip) or re.match(r'^[a-fA-F0-9]{40}$', ip) or re.match(r'^[a-fA-F0-9]{64}$', ip):
                hash_node_count += 1
print(f"  Graph node (hash → IPAddress): {hash_node_count}")
print(f"  Hash entity → graph node oranı: {hash_node_count}/{hash_entity_count} ({hash_node_count*100//max(hash_entity_count,1)}%)")
print(f"  Hash entity'lerin merge edilme oranı: {hash_entity_count - hash_node_count}/{hash_entity_count} ({(hash_entity_count - hash_node_count)*100//max(hash_entity_count,1)}%)")
...
for inc in result.incidents:
    ...
    print(f"    {inc_id}: [{inc.get('category')}] {len(steps)} steps, entity={expl.get('primary_entity','')[:40]}")
