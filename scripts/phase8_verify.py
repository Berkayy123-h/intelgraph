...
for te in result.get("truth_entries", []):
    ...
    action = te.get("truth", {}).get("action", "written") if isinstance(te.get("truth"), dict) else "written"
    ssot = te.get("ssot", "")
    if not key:
        continue

    ev = Evidence(
        id=f"ev_{hash(key)%10**8:08x}", source=str(ssot),
        content=action, collected_at=now,
        source_tier=2, trust_score=70, reliability_score=70,
    )
...
if len(nids) >= 2:
    ...
    rel = Relationship(
        ...
        type=RelationshipType.RELATED_TO, confidence_score=80,
    )
...
if nids:
    ...
    g1 += chk(f"neighbors() returns {len(neigh)} node(s)", True,
...
g1 += chk(f"node_count == {graph.node_count} matches len(nodes)",
          graph.node_count == len(graph.nodes))
g1 += chk(f"edge_count == {graph.edge_count} matches len(edges)",
...
if nids:
    ...
    g1 += chk(f"extract_subgraph(depth=1) → {sub.node_count} node(s)",
              sub.node_count >= 1)
...
result_ut = subprocess.run(
    ...
    capture_output=True, text=True, cwd="/home/berkay/intelgraph",
)
...
r = api("POST", "/auth/register",
        headers={"Content-Type": "application/json"},
        data=json.dumps({"username": "phase8_user", "password": "phase8_pass", "role": "admin"}).encode())
a += chk("POST /auth/register (admin) → 200/201", r["status"] in (200, 201),
...
a += chk("POST /auth/login → 200/201 with access_token",
...
if token:
    r = api("POST", "/entities",
            ...
            data=json.dumps({"entity_type": "domain", "attributes": {"domain_name": "test.example.com"}}).encode())
    a += chk("POST /entities with valid admin token → 200/201",
...
rl += chk("Rate limit headers on GET /health", has_rl,
...
rl += chk(f"Auth rate limit headers present (limit={auth_limit}, remaining={auth_remaining})",
...
if got_429:
    ...
    r = api("POST", "/auth/login",
            headers={"Content-Type": "application/json"},
            data=json.dumps({"username": "phase8_user", "password": "phase8_pass"}).encode())
    rl += chk("Retry-After header on 429",
...
print(f"  Kalan: neighbors/outgoing/incoming/has_edge — yardimci metodlar,")
print(f"  add_relationship/get_node/bfs/dfs/shortest_path/digerleri zaten testli.")
print(f"  api.main: Auth akisi calisiyor (401/200). Rate limit calisiyor (429).")
