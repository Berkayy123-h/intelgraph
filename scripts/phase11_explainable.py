...
for inc in result.incidents:
    print(f"    [{inc.get('alert_id','?')}] {inc.get('category','?')} — {inc.get('message','')[:80]}")
...
print(f"  Mevcut veri kaynaklari:")
...
for r in result_dict.get("relationships", [])[:3]:
    print(f"    {r.get('relation','?')}: {r.get('subject','?')} -> {r.get('object','?')} "
...
for gn in gsummary[:3]:
    print(f"      {gn['node_id'][:30]}: type={gn['entity_type']}, id={gn.get('entity_identifier','?')[:25]}, "
          f"conf={gn['confidence']}, evidence={gn['evidence_count']}")
ma = result_dict.get("merge_audit", [])
print(f"    merge_audit: {len(ma)} kayit")
for m in ma[:2]:
    print(f"      {m.get('source_entity_id','?')} -> {m.get('target_entity_id','?')} "
...
for inc in result.incidents:
    ...
    for step in explanation["steps"]:
        phase_icon = {
            "source_ingestion": "📥", "truth_estimation": "📊", "contradiction": "⚡",
            "relationship": "🔗", "evidence_chain": "🔬", "alert": "🚨",
            "safety_governor": "🛡️", "human_review": "👤", "incident": "📋",
        }.get(step["phase"], "•")
...
if not cve_found:
    print("  Incidents alert ile iliskilendirilemedi (beklenen — ICC incident'lari entity context'ine sahip degil)")
...
report = {
    ...
    "explanation_test": {
        ...
        "incidents_explained": len([i for i in result.incidents if "error" not in builder.explain(i.get("alert_id","?"))]),
...
# ── Summary ──
print(f"\n{'='*72}")
print(f"  FAZ 11 TAMAM")
print(f"  Pipeline: {len(result.extracted_entities)} entity, {len(result.incidents)} incident")
print(f"  ExplanationBuilder: {len(result.incidents)}/{len(result.incidents)} incident aciklamasi olusturuldu")
print(f"  Eksik veri: entity_id in incident (ICC), graph JSON serialization, ResolutionAudit")
