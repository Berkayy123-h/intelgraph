...
ip_src = IPAddress(
    ...
    ip="10.0.0.2",
    evidence=(
        Evidence(id="ev_c1", source="src1", content="original", collected_at=datetime.now(timezone.utc),
                 source_tier=1, trust_score=80, reliability_score=80),
    ),
)
...
ip_dst = IPAddress(
    ...
    ip="10.0.0.2",
    evidence=(
        Evidence(id="ev_c2", source="src2", content="updated", collected_at=datetime.now(timezone.utc),
                 source_tier=1, trust_score=90, reliability_score=90),
    ),
)
...
from intelgraph.core.evidence_chain import EvidenceChain, EvidenceItem, SupportType, ContradictionDetector

print("--- EvidenceChain + ContradictionDetector ---")
chain = EvidenceChain(entity_id="test_ip_10_0_0_1")
chain.add_item(EvidenceItem(
    source_id="src_a", claim="IP zararsız",
    support_type=SupportType.SUPPORTS, confidence=80.0,
))
chain.add_item(EvidenceItem(
    source_id="src_b", claim="IP zararlı",
    support_type=SupportType.CONTRADICTS, confidence=90.0,
))
...
chain2.add_item(EvidenceItem(
    source_id="misp", claim="IP C2 sunucusu", support_type=SupportType.SUPPORTS, confidence=90.0,
))
chain2.add_item(EvidenceItem(
    source_id="vt", claim="IP C2 sunucusu", support_type=SupportType.SUPPORTS, confidence=80.0,
))
chain2.add_item(EvidenceItem(
    source_id="alienvault", claim="IP zararsız", support_type=SupportType.CONTRADICTS, confidence=70.0,
))

source_trust = {"misp": 95, "vt": 85, "alienvault": 60}
final_conf = ConfidenceComputer.compute(chain2, source_trust)

print(f"3 adet evidence:")
for item in chain2.evidence:
    print(f"  {item.source_id:15s} claim='{item.claim:20s}' {item.support_type.name:12s} conf={item.confidence}")
...
# Adım adım hesaplama
print(f"\nHesap adımları:")
print(f"  1. Weighted sum = Σ(confidence × source_trust/100)")
print(f"  2. source_trust ile normalize")
print(f"  3. Eğer agreement_ratio > 0.7 → %10 boost")
print(f"  4. contradiction_ratio > 0.3 → penalty")
print(f"  (ConfidenceComputer kaynağı: evidence_chain/confidence.py)")
...
print(f"   Gerçek bir agregasyon mantığı VAR.")
...
e_with_evidence = IPAddress(
    ...
    ip="10.0.0.3",
    evidence=(
        Evidence(id="ev_g1", source="test", content="graph evidence",
...
print(f"  EvidenceChain'e bağlı mı? HAYIR (entity'ye gömülü).")
