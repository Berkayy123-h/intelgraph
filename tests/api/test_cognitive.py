...
class TestCognitiveEndpoints:
    ...
    def _auth_headers(self, role: str = "analyst"):
        resp = self.client.post("/auth/register", json={
            "username": f"{role}1", "password": "test123", "role": role,
        })
    ...
    def test_reasoning_query(self):
        ...
        resp = self.client.post("/reasoning/query", json={
            "start": "A", "end": "B", "type": "multi_hop",
        }, headers=headers)
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_reasoning_explain_nonexistent(self):
        headers = self._auth_headers()
        resp = self.client.post("/reasoning/explain", json={
    ...
    def test_hypothesis_generate(self):
        headers = self._auth_headers()
        resp = self.client.post("/reasoning/hypothesis/generate", json={
            "graph": None,
        }, headers=headers)
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_learning_feedback(self):
        headers = self._auth_headers()
        resp = self.client.post("/reasoning/learning/feedback", json={
            "query_id": "q1",
    ...
    def test_validate_hypothesis(self):
        headers = self._auth_headers()
        resp = self.client.post("/reasoning/validate", json={
    ...
    def test_detect_contradictions(self):
        headers = self._auth_headers()
        resp = self.client.post("/reasoning/contradictions/detect", json={
            "facts": [
    ...
    def test_authz_rejection(self):
        ...
        resp = client.post("/reasoning/validate", json={
            "hypothesis_id": "h1", "confidence": 0.5,
        }, headers=headers)
