...
class TestNLPEndpointsUnauthenticated:
    ...
    def test_extract_entities(self):
        ...
        resp = self.client.post("/nlp/extract-entities", json={
    ...
    def test_extract_relationships(self):
        headers = self._auth_headers()
        resp = self.client.post("/nlp/extract-relationships", json={
    ...
    def test_extract_events(self):
        headers = self._auth_headers()
        resp = self.client.post("/nlp/extract-events", json={
    ...
    def test_classify_text(self):
        headers = self._auth_headers()
        resp = self.client.post("/nlp/classify-text", json={
    ...
    def test_summarize_document(self):
        headers = self._auth_headers()
        resp = self.client.post("/nlp/summarize-document", json={
    ...
    def test_link_to_graph(self):
        headers = self._auth_headers()
        resp = self.client.post("/nlp/link-to-graph", json={
    ...
    def test_ingest_text(self):
        headers = self._auth_headers()
        resp = self.client.post("/nlp/ingest-text", json={
    ...
    def test_analyze_roi(self):
        headers = self._auth_headers()
        resp = self.client.post("/nlp/analyze-roi", json={
            "query_id": "test-1", "estimated_value": 100, "estimated_cost": 10,
        }, headers=headers)
        assert resp.status_code == 200
        data = resp.json()
        assert "roi" in data
