...
class TestReasoningEngine:
    ...
    def test_reasoning_step_uncertainty(self):
        ...
        step = ReasoningStep(
            step_id="s1", source_node="A", target_node="B", relation="connects",
            confidence=0.8, evidence=["test"], uncertainty=0.2, step_type="direct",
        )
        d = step.to_dict()
...
class TestSelfLearningLoop:
    def test_ingest_feedback(self):
        ...
        entry = loop.ingest_feedback("q1", "analyst_1", "correction", 0.8, {"fix": "x"}, {"original": "y"})
...
class TestContinuousOptimizer:
    ...
    def test_optimize_routing(self):
        from intelgraph.core.cognitive.optimization import ContinuousOptimizer
        opt = ContinuousOptimizer()
        opt.record_optimization(type("OM", (), {"extraction_accuracy": 0.8, "model_routing_efficiency": 0.7, "reasoning_cost": 0.5, "throughput": 100, "drift_score": 0.05, "threshold_sensitivity": 0.5})())
    ...
    def test_detect_drift(self):
        ...
        opt = ContinuousOptimizer()
        for i in range(15):
            opt.record_optimization(OptimizationMetrics(
                extraction_accuracy=0.8 + (i % 5) * 0.05,
                model_routing_efficiency=0.7, reasoning_cost=0.5,
                throughput=100, drift_score=0.05, threshold_sensitivity=0.5,
            ))
...
class TestCognitiveIntegration:
    ...
    def test_learning_with_feedback(self):
        from intelgraph.core.cognitive.learning import SelfLearningLoop
        loop = SelfLearningLoop()
        loop.ingest_feedback("q1", "analyst", "correction", 0.9, {"correct": "val"}, {"original": "wrong"})
        loop.ingest_feedback("q2", "analyst", "correction", 0.7, {"correct": "val"}, {"original": "wrong"})
    ...
    def test_full_cognitive_pipeline(self):
        ...
        assert len(paths) >= 1
        for p in paths:
            trace.record(f"{p.start_node} -> {p.end_node}", [s.to_dict() for s in p.steps], [], p.evidence_chain, p.score)
        hypotheses = gen.generate(mock_graph)
        assert len(hypotheses) >= 1
